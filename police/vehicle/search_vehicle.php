        ...
        <div class="row d-flex justify-content-center align-items-center h-100">
            <div class="col-md-7 col-lg-5 col-xl-3">
                <form action="" method="GET" class="d-flex" role="search">
                    <input type="text" name="search" required value="<?php if(isset($_GET['search'])){echo $_GET['search']; } ?>" class="form-control" placeholder="Enter Vehicle's Licence Number">
                            ...
                            <table  class="table table-striped table-bordered border-5 border-dark">
                                ...
                                <tbody class="table-group-divider">
                                    <?php                
                                        ...
                                        $value = $_GET['search'];
                                        $query = "SELECT * FROM vehicle v LEFT JOIN ownership o ON v.Vehicle_ID = o.Vehicle_ID LEFT JOIN people p ON p.People_ID = o.People_ID  WHERE v.vehicle_licence LIKE '%$value%'";
                                        $result = mysqli_query($conn,$query);

                                        $Date = date("Y-m-d H:i:s");
                                        $user = $_SESSION['username'];
                                        $audit = "INSERT INTO audit VALUES ('','$user','Search','Vehicle','$value','$Date')";
                                        $audit_query = mysqli_query($conn,$audit);
